...
tf.random.set_seed(
    777
)  # 하이퍼파라미터 튜닝을 위해 실행시 마다 변수가 같은 초기값 가지게 하기
...
def build_test_residual(input_dim=4, output_dim=1, model_name="residual_model"):
    inputs = tf.keras.Input(shape=(input_dim,))
    block_unit = [30, 30]
    ...
    outputs = layers.Dense(output_dim)(x)
    outputs = keras.activations.softplus(outputs)

    model = Model(inputs, outputs, name=model_name)
    return compile_model(model, optimizers.Adam(1e-3))


def build_bnn_residual(
    input_dim=4, output_dim=1, model_name="bnn_residual", dataset_size=1000
):

    inputs = keras.Input(shape=(input_dim,))

    kl_weight = 1.0 / dataset_size

    x = tfpl.DenseVariational(
        256, make_prior_fn=prior_fn, make_posterior_fn=posterior_fn, kl_weight=kl_weight
    )(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.LeakyReLU()(x)

    x = tfpl.DenseVariational(
        128, make_prior_fn=prior_fn, make_posterior_fn=posterior_fn, kl_weight=kl_weight
    )(x)
    x = layers.BatchNormalization()(x)
    x = layers.LeakyReLU()(x)
...
def split_data(*arrays, train_ratio=0.70, val_ratio=0.15, random_state=42):
    ...
    temp_split = train_test_split(
        *arrays, test_size=1 - train_ratio, random_state=random_state
    )
    ...
    val_test_split = train_test_split(
        *temps,
...
def train_and_evaluate(
    ...
    reduce_lr = ReduceLROnPlateau(
        monitor="val_loss", factor=0.5, patience=5, min_lr=1e-6  # lr 절반으로 감소
    )
...
def train_and_evaluate_sklearn(
    model, X_train, y_train, X_val, y_val, X_test, y_test, model_name="model"
):
...
def generate_features(X):
    ...
    L_ratio = L1 / (L2 + epsilon)
    C_ratio = C1 / (C2 + epsilon)

    custom = np.column_stack(
        [LC1, LC2, 1 / (LC1 + epsilon), 1 / (LC2 + epsilon), L_ratio, C_ratio]
    )  # (N, 6)
