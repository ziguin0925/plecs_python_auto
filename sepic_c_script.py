...
try
    ...
    idx = find(time_list >= target, 1);

    if isempty(idx)
        error("해당 시간 없음");
    endif

    result = [idx, time_list(idx)];
...
param_specs = {
    ...
    "L2": {"start": 50e-6, "step": 50e-6, "count": 1},
    "C1": {"start": 5e-6, "step": 50e-6, "count": 1},
    "C2": {"start": 188e-6, "step": 5e3, "count": 1},
}
...
def run_simulation(opts, plecs, name):
    try:
        start = time.time()
        result = plecs.simulate(name, opts, callback)
